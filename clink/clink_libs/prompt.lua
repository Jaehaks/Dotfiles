...
---@class git_state
---@field check_dir string directory where git checking process is started
---@field prompt string prompt includes only git status
---@field running boolean Flag whether git status checking is running
local git_state = {
	check_dir = "",
	prompt = "",
	running = false,
...
function git_branch_prompt:filter(prompt)
	...
	-- if directory is moved before git status checking is completed,
	local curdir = os.getcwd()
	if curdir == git_state.check_dir then
		return prompt .. git_state.prompt
	end

	-- if git checking is running, the result is not shown.
	if git_state.running then
		return prompt
	end
	...
	-- git check using synchronous method
	clink.promptcoroutine(function ()
		git_state.check_dir = os.getcwd()
		local status = {
			branch          = nil,
			oid             = nil,
			-- after modified
		...
		-- check status
		-- use popenyeild() to asynchronous operation in coroutine
		local pipe = io.popenyield('git status --porcelain=v2 --branch --ignored=matching --show-stash 2>nul')
		if not pipe then
			return
		end
		...
		if status.branch then
			...
			if (status.stagingAdded + status.stagingRenamed + status.stagingCopied + status.stagingDeleted + status.stagingConflict + status.commit + status.behind) == 0 then
				git_state.prompt = ' ' .. git_status_branch .. git_status_working .. normal
			else
				git_state.prompt = ' ' .. git_status_branch .. git_status_working .. git_status_staging .. normal
			end
			clink.refilterprompt() -- redraw prompt. it calls all prompt filter again after coroutine is finished.
		end
		git_state.running = false
	end)

	-- show empty prompt while waiting git checking result
	return prompt
end
